...
{
    ui->setupUi(this);
    // ui->videoWidget2->setStyleSheet("background-color: green;");
    connect(ui->closeBtn, &QPushButton::clicked, this, &VideoWidget::sig_CloseBtnClick);
...
void VideoWidget::setIsPlaying(bool value)
{
    mIsPlaying = value;
...
void VideoWidget::setPlayFailed(bool value)
{
    mPlayFailed = value;
    QMetaObject::invokeMethod(this, [=]() {
        update();
...
void VideoWidget::setCameraName(QString name)
{
    mCameraName = name;
    QMetaObject::invokeMethod(this, [=]()
    {
...
void VideoWidget::setVideoWidth(int w, int h)
{
    if (w <= 0 || h <= 0) {
        return;
    }
...
void VideoWidget::setCloseAble(bool isCloseAble)
{
...
void VideoWidget::inputOneFrame(VideoRawFramePtr videoFrame)
{
    QMetaObject::invokeMethod(this, [=]() {
...
void VideoWidget::resizeGL(int window_W, int window_H)
{
    ...
    int x = window_W - ui->closeBtn->width() - 22;
    int y = 22;
    ui->closeBtn->move(x, y);

    x = 0;
    y = window_H / 2 - ui->errorWidget->height() / 2;
    ui->errorWidget->move(x, y);
    ui->errorWidget->resize(window_W, ui->errorWidget->height());

    x = 0;
    y = window_H - ui->nameWidget->height() - 6;
    ui->nameWidget->move(x, y);
    ui->nameWidget->resize(window_W, ui->nameWidget->height());
...
void VideoWidget::paintGL()
{
    ...
    if (!mCameraName.isEmpty() && mIsPlaying) {
        ui->nameWidget->show();
        QFontMetrics fontMetrics(ui->nameLabel->font());
        ...
        ui->nameLabel->setToolTip(mCameraName);
    } else {
        ui->nameWidget->hide();
    }
